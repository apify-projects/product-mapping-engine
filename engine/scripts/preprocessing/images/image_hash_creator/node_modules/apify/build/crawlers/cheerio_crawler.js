...
class CheerioCrawler extends basic_crawler_1.BasicCrawler {
    ...
    async _handleNavigation(crawlingContext) {
        ...
        crawlingContext.response = await utils_1.addTimeoutToPromise(this._requestFunction({ request, session, proxyUrl, requestAsBrowserOptions }), this.requestTimeoutMillis, `request timed out after ${this.requestTimeoutMillis / 1000} seconds.`);
    ...
    async _parseResponse(request, responseStream) {
        ...
        if (statusCode >= 500) {
            const body = await utilities_1.readStreamToString(response, encoding);
            ...
            const body = await utilities_1.concatStreamToBuffer(response);
