...
const { BROWSER_POOL_EVENTS: { BROWSER_LAUNCHED, BROWSER_RETIRED, PAGE_CREATED, PAGE_CLOSED, }, } = require('./events');
...
class BrowserPool extends EventEmitter {
    constructor(options = {}) {
        ...
        const { browserPlugins, maxOpenPagesPerBrowser = 20, retireBrowserAfterPageCount = 100, operationTimeoutSecs = 15, closeInactiveBrowserAfterSecs = 300, preLaunchHooks = [], postLaunchHooks = [], prePageCreateHooks = [], postPageCreateHooks = [], prePageCloseHooks = [], postPageCloseHooks = [], } = options;
        super();
        ...
        this.browserKillerInterval = setInterval(() => this._closeInactiveRetiredBrowsers(), BROWSER_KILLER_INTERVAL_MILLIS);
    }
    /**
    ...
    async newPage(options = {}) {
        const { id = nanoid(), pageOptions, browserPlugin = this._pickBrowserPlugin(), } = options;
    ...
    async _createPageForBrowser(pageId, browserController, pageOptions = {}) {
        ...
        let page;
        try {
            page = await addTimeoutToPromise(browserController.newPage(finalPageOptions), this.operationTimeoutMillis, 'browserController.newPage() timed out.');
